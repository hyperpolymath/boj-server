...
The Cartridge Configurator is a tool for applying runtime configuration
to cartridges dynamically.
...
The Cartridge Configurator applies the configuration to the cartridge
and writes it to a `+config.json+` file in the cartridge directory. If
hot-reload is supported, it triggers a hot-reload of the cartridge.
