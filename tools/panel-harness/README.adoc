...
The Panel Harness is a tool for bridging cartridges to BoJ Server and
panll.
...
* `+--routes <routes-file>+`: Path to the routes configuration file for
the cartridge
...
The Panel Harness registers the cartridge with the specified target and
writes the registration to a `+registration.json+` file in the harness
directory.
