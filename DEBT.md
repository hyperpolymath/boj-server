...
| C-3 | HIGH | **`.github/workflows/main-estate-audit.yml` is untracked**, has **no `permissions:` block** (the only workflow in the repo without one), and pins **27 actions to `@main` / `@v4`** rather than SHAs. It has therefore never run. Same file, same defects, in the sibling repo. | `git status --porcelain .github/workflows/` · `grep -c '@main' .github/workflows/main-estate-audit.yml` |
...
| D-7 | LOW | TODO/FIXME/XXX/HACK density is genuinely near zero — all 63 matches are policy/tooling references to marker *scanning*, not markers. Recorded as a positive control. | `git grep -nE '\b(TODO\|FIXME\|XXX\|HACK)\b' \| wc -l` |
