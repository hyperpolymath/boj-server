...
//
// turso_mcp_adapter.v — V-lang REST adapter for the turso-mcp cartridge.
//
// Bridges the Zig FFI C-ABI exports to the unified BoJ adapter protocol.
// Auth: Bearer token (Turso API token).
// API base: https://api.turso.tech/v1/ + libSQL client URL.
// Edge-replica-aware with embedded replica support.

module turso_mcp_adapter
...
fn C.turso_mcp_can_transition(from int, to int) int
fn C.turso_mcp_session_open() int
fn C.turso_mcp_session_close(slot_idx int) int
fn C.turso_mcp_session_state(slot_idx int) int
fn C.turso_mcp_begin_query(slot_idx int) int
fn C.turso_mcp_end_query(slot_idx int) int
fn C.turso_mcp_signal_error(slot_idx int) int
fn C.turso_mcp_query_count(slot_idx int) int
fn C.turso_mcp_action_requires_connection(action int) int
fn C.turso_mcp_reset()
...
pub fn session_close(slot int) !string {
	result := C.turso_mcp_session_close(slot)
	return match result {
		0 { 'closed slot ${slot}' }
...
pub fn end_query(slot int) !string {
	result := C.turso_mcp_end_query(slot)
	return match result {
