...
export fn vext_verify_message(msg: [*c]const u8, sig: [*c]const u8) callconv(.C) u8 {
...
export fn vext_check_attestation(issuer: [*c]const u8) callconv(.C) u32 {
    if (issuer == null) return 0;
    return 1; // Stub
}

/// Append an entry to the verification chain. Returns 0 on success.
export fn vext_append_chain(payload: [*c]const u8) callconv(.C) i32 {
