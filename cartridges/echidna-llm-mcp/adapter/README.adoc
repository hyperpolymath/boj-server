...
= echidna-llm-mcp / adapter — protocol bridge
:orientation: shallow
...
| `echidna_llm_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `echidna_llm_adapter.zig` — dispatch function and per-protocol routers.
