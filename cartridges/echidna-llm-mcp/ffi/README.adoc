...
= echidna-llm-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libechidna-llm.so`.
| `echidna_llm_ffi.zig`     | C-ABI exports (11 exports, 3 inline tests, 359 lines).
|===
...
3 inline `test "..."` block(s) in `echidna_llm_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `echidna_llm_ffi.zig` — C-ABI signatures and guards.
