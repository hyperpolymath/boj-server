...
export fn opsm_connect(slot_idx: u32) callconv(.C) i32 {
...
export fn opsm_start_query(slot_idx: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
    const slot = &slots[slot_idx];
...
export fn opsm_end_query(slot_idx: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
    const slot = &slots[slot_idx];
...
export fn opsm_reset(slot_idx: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
    const slot = &slots[slot_idx];
...
export fn opsm_disconnect(slot_idx: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
    const slot = &slots[slot_idx];
...
export fn opsm_state(slot_idx: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
    return @intCast(@intFromEnum(slots[slot_idx].state));
}

/// Check if a state transition is valid.
export fn opsm_can_transition(from: u8, to: u8) callconv(.C) i32 {
...
export fn opsm_reset_all() callconv(.C) void {
    for (&slots) |*slot| {
        slot.state = .disconnected;
        slot.name_len = 0;
    }
...
export fn opsm_set_name(slot_idx: u32, name_ptr: [*]const u8, name_len: u32) callconv(.C) i32 {
    if (slot_idx >= MAX_REGISTRIES) return @intFromEnum(OpsmError.invalid_slot);
