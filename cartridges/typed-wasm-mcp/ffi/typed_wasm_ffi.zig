...
export fn typed_wasm_validate_module(module_path: [*c]const u8) callconv(.C) u8 {
    if (module_path == null) return 255;
    return 5; // Stub — mid-level safety
}

/// Check types in a module. Returns error count.
export fn typed_wasm_check_types(module_path: [*c]const u8) callconv(.C) u32 {
    if (module_path == null) return 1;
    return 0; // Stub — no errors
}

/// Compile a module. Returns 0 on success, -1 on error.
export fn typed_wasm_compile_module(module_path: [*c]const u8, target: u8) callconv(.C) i32 {
    if (module_path == null) return -1;
