...
//
// Three-protocol BoJ adapter: REST (port 9392), gRPC-compat (port 9393),
// GraphQL (port 9394).
// Replaces the banned V-lang adapter (k9iser_adapter.v).

const std = @import("std");
const ffi = @import("k9iser_ffi");

const REST_PORT: u16 = 9392;
const GRPC_PORT: u16 = 9393;
const GQL_PORT:  u16 = 9394;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "k9_load_manifest")) {
        return .{ .status = 200, .body = okJson(resp, "k9_load_manifest forwarded") };
    }
    if (std.mem.eql(u8, tool, "k9_generate")) {
        return .{ .status = 200, .body = okJson(resp, "k9_generate forwarded") };
    }
    if (std.mem.eql(u8, tool, "k9_validate")) {
        return .{ .status = 200, .body = okJson(resp, "k9_validate forwarded") };
    }
    if (std.mem.eql(u8, tool, "k9_apply")) {
        return .{ .status = 200, .body = okJson(resp, "k9_apply forwarded") };
    }
    if (std.mem.eql(u8, tool, "k9_clean")) {
        return .{ .status = 200, .body = okJson(resp, "k9_clean forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "k9_load_manifest") != null)
        return dispatch("k9_load_manifest", body, resp);
    if (std.mem.indexOf(u8, body, "k9_generate") != null)
        return dispatch("k9_generate", body, resp);
    if (std.mem.indexOf(u8, body, "k9_validate") != null)
        return dispatch("k9_validate", body, resp);
    if (std.mem.indexOf(u8, body, "k9_apply") != null)
        return dispatch("k9_apply", body, resp);
    if (std.mem.indexOf(u8, body, "k9_clean") != null)
        return dispatch("k9_clean", body, resp);
