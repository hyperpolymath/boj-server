...
| *ABI*
| Idris2
| Dependent-type proofs, state machines, `%default total`, zero `believe_me`
| `src/abi/` (core), `cartridges/*/abi/`

| *FFI*
| Zig
| C-compatible native execution, zero runtime dependencies
| `ffi/zig/` (core), `cartridges/*/ffi/`

| *Adapter*
| Elixir
| Triple API: REST (7700) + gRPC (7701) + GraphQL (7702)
| `elixir/`
...
[source,bash]
----
guix shell -D -f build/guix.scm    # Guix (primary)
guix develop                    # Guix (fallback)
----
...
See link:../EXTENSIBILITY.md[`docs/EXTENSIBILITY.adoc`] for the full guide.
...
| Core FFI
| 178
| Catalogue, loader, federation, guardian, readiness, VeriSimDB, e2e, coprocessor, SLA, community, SDP, seams

| Cartridge FFI
| 118
| Per-cartridge Zig tests

| Multi-node federation
| 11
| REST API peer management, gossip
...
. *Every mutable global* must have a corresponding mutex
. *Every exported function* that touches mutable state must lock before access
. *Use `defer mutex.unlock()`* to guarantee unlock on all code paths
. *Never hold two mutexes* at the same time (prevents deadlocks)
. *Keep critical sections short* -- do computation outside the lock when possible
...
See link:../../CONTRIBUTING.md[`CONTRIBUTING.md`] for the full contribution guide, including commit conventions, PR requirements, and the gitbot-fleet review process.
