...
Reusable template for a fast Haiku scouting pass over a concrete file
list after a coord task lands. Paired with the multi-agent coord flow
...
* Scope must be an *exact file list* (no globs, no "`the changed
files`") so the scout has a finite surface.
...
* The `+<20 lines+` cap keeps the report digestible in the master’s next
batch-review slot.
...
[arabic]
. `+coord_register+` as `+kind=claude+`, `+variant=haiku-4.5+`,
`+declared_affinities=["scout", "lint"]+`.
. `+coord_claim_task+` with `+task_difficulty=routine+`,
`+dispatch_preference=auto+`.
. For each flagged item in step 4, emit `+coord_send_gated+` with
`+risk_tier=1+` (warn) — goes straight into the master’s quarantine.
. On completion, `+coord_report_outcome+` with the observed tag and
